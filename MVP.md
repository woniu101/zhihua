...
8. 为每个分镜选择正式版本，仅对正式版本制作 1080p 成片。
9. 调整简单的时间线、字幕、转场和音量。
...
- 成片处理：仅对已选正式版本使用 SeedVR2 3B FP8 做时序超分和精确尺寸输出。
- 视频合成：桌面端调用 FFmpeg。
...
未来可以通过 `ImageProvider` 接入 Qwen-Image-Edit 等模型，但不进入当前开发范围。公共模型库已实测存在图片生成、编辑模型，只作为后续扩展条件，不因此扩大本次 MVP 功能范围。
...
| 1080p 成片档案 | SeedVR2 3B FP8 及对应节点 | 以固定清单为准 | MVP 必须 |
...
- H3 内部宽高必须满足 32 的倍数。后端在安全像素预算内选择最接近的生成尺寸，并通过轻微裁切或留边适配最终画幅；用户只选择成片比例，不需要理解模型内部尺寸。
- 分镜生成时长固定为 5、10、15 秒。模型的实际帧时长允许略有偏差，进入时间线后通过末尾微裁切、静帧保持或短淡化对齐旁白和项目时长。
- MVP 最高交付规格为 1920×1080。已选正式版本才进入 SeedVR2 时序超分；16:9 输出 1920×1080，9:16 输出 1080×1920，4:3 输出 1440×1080，3:4 输出 1080×1440，1:1 输出 1080×1080。
...
flowchart LR
    ...
    C --> D[提交 H3 候选任务并保存 prompt_id]
    D --> E[下载候选结果]
    E --> F{用户是否制作正式 1080p 成片}
    F -- 是 --> G[SeedVR2 时序超分并下载]
    F -- 否 --> H{队列是否为空}
    G --> H
    H -- 否 --> D
    H -- 是 --> I[结果落盘后空闲 3 分钟关机]
```
...
1. 用户先用快速 8 步或高质量 20 步生成候选。
2. 候选卡片显示提示词、seed、工作流版本、生成尺寸和音轨状态，支持对比播放。
3. “设为正式版本”只改变项目选择，不触发生成。
4. “制作 1080p 成片”仅处理当前正式版本；20 步重新生成会改变内容，不能包装成对原片的无损升级。
5. 断线或应用重启后先依据已保存的 `prompt_id` 查询远端状态，禁止重复提交同一任务。
...
| 1080p 成片 | 放大已选正式版本并输出精确尺寸 | SeedVR2 3B FP8、时序超分、分块处理 |

20 步会重新采样并改变画面内容，不能显示为“高清修复”。1080p 成片不会替代候选选择，默认也不处理未入选版本。具体节点参数放在版本化模板中，不作为普通 UI 设置；分块、重叠和显存参数仅用于诊断。
...
- SeedVR2 3B FP8、对应自定义节点和 `seedvr2-1080p-v1.json` 全部就绪后，才启用“制作 1080p 成片”。
...
Turbo 与 SeedVR2 均必须在清单中列出固定版本、兼容基础权重、自定义节点和工作流；不能因为找到文件就视为链路已验收。
...
- 提交候选任务并下载、预览视频，选择正式版本后制作 1080p 成片。
...
- 快速候选/高质量候选，以及独立的 1080p 成片处理。
- 取消、重试和故障恢复。
- 多版本选择。
- H3 首帧、首尾帧和参考生成。
- 实例保留期限提示与可选无卡维护，完成回收期效果验收后再开放自动执行。

验收：用户可以为全部分镜生成旁白和视频，并逐一选择正式版本；默认只启动一个 worker，开启弹性多路后可将不同分镜分配给多个可用 worker，失败或重启不重复提交，同一分镜不会被两个 worker 同时处理。
...
- [ ] 每个分镜可以保留多个版本并选择正式版本。
- [ ] 只有正式版本可进入 SeedVR2，能够输出精确 1080p 尺寸并在失败后保留原候选。
...
- [ ] 生成、下载、1080p 处理和自动关机阶段中断后可恢复，不重复提交任务。
...
实现时优先遵循本文档的功能范围。分镜页重点表达项目画幅、5/10/15 秒、四种 H3 输入方式、候选版本、正式版本、1080p 成片、真实任务阶段和顶栏轻量算力状态。设置与算力页使用顶部横向分类，承担余额、实例保留期维护、自动关机和环境检查；素材页不增加图片生成入口，导出页不显示项目费用。
...
| 图片扩展条件 | 公共库中的 Qwen Image 2512、Qwen Image Edit 2511 及所需编码器/VAE 已完成真实文生图与图片编辑推理；能力可用，但不进入本次 MVP |
...
| 1080p | 1344×768 候选经画幅适配后，使用 SeedVR2 3B FP8、batch 13、overlap 3 输出精确 1920×1080，约 174.17 秒，峰值显存约 23.47GiB；片段边界稳定性检查通过 |
...
1. 用全新账号、全新客户端和全新实例走通“配置凭据 → 自动准备环境 → 生成 → 1080p → 下载”，全程不打开 ComfyUI 界面。
2. 完成一条真实 30～90 秒、多分镜的科普成片，验收系统 TTS、字幕、音乐、转场、精确时长和 FFmpeg 导出。
3. 在生成、下载、1080p 处理和自动关机阶段分别制造客户端退出、断网、502 与实例重启，确认按 `prompt_id` 恢复且不重复提交。
...
无卡循环延长七天回收期仍缺少平台可核验证据，自动维护开关保持禁用；只提供只读期限提示、手动控制和平台入口。图片生成与编辑虽已完成底层推理验证，仍在 `ImageProvider` 后续范围，不阻塞当前视频 MVP。
...
- 本地 FFmpeg 已真实完成 1920×1080 H.264/AAC 规范化、镜头拼接、旁白混音、可选背景音乐循环与淡入淡出、字幕烧录和 SRT 输出；H3 环境音保持禁用，直到试听与保留流程完成 GPU 验证。
- 桌面端单元测试、前端类型检查与生产构建通过；真实 Windows 系统语音和带背景音乐的 FFmpeg 合成测试通过。当前证据不等同于 30～90 秒真实 H3 多分镜成片验收。

### 22.4 2026-09-11 客户端到 5090 端到端实测

- 乌兰察布 96GB 内存的 RTX 5090 实例上，通过本地 SSH 隧道调用 `zhihua-service 0.3.0` 新建 H3 T2V Turbo 任务；同一 `client_request_id` 重试返回同一任务，确认服务端幂等生效。
...
- 修复桌面端对服务端 snake_case 结果清单的解析；原生 Rust 客户端已通过真实服务的兼容握手、10 条工作流能力读取、已完成任务恢复和结果清单反序列化。
- 本轮仍是单镜头链路验证；多镜头连续运行、断网/重启恢复、弹性多实例分发和最终自动关机验收继续按 22.2 执行。
