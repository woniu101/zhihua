...
function shotRenditionLabel(shotId: string): string {
  if (!readyShotIds.value.includes(shotId)) return "未就绪";
  if (settings.rendition === "enhanced-1080p") {
    return enhancedShotIds.value.includes(shotId) ? "AI 增强" : "候选放大";
  }
  return "正式候选";
}

function resolveIntegrityItem(itemId: string) {
  ...
  const target = routeByItem[itemId];
    ...
    <div class="export-layout">
      ...
      <aside class="export-right">
        <section class="panel readiness-panel">
          ...
          <div class="check-grid"><p v-for="item in integrityItems" :key="item.id" :class="{failed:!item.passed}" :role="item.passed ? undefined : 'button'" :tabindex="item.passed ? undefined : 0" :title="item.passed ? item.detail : `${item.detail}，点击去处理`" @click="!item.passed && resolveIntegrityItem(item.id)" @keydown.enter="!item.passed && resolveIntegrityItem(item.id)"><span class="check"><Check v-if="item.passed" :size="13"/><X v-else :size="13"/></span><span><b>{{ item.label }}</b><small>{{ item.detail }}<em v-if="!item.passed">去处理</em></small></span></p></div>
        ...
        <section class="panel export-settings">
          ...
          <div class="settings-grid">
            ...
            <label><span><Monitor :size="15"/>输出清晰度</span><select v-model="settings.rendition"><option value="candidate">候选原清晰度（快速）</option><option value="enhanced-1080p">1080p（逐镜头混合增强）</option></select></label>
...
<style scoped>
.export-page{display:grid;grid-template-rows:64px minmax(0,1fr);gap:10px}.export-head{display:flex;align-items:center;justify-content:space-between;padding:0 6px}.ready{display:grid;grid-template-columns:24px auto;gap:1px 9px;align-items:center}.ready .status-symbol{grid-row:1/3;width:22px;height:22px;border-radius:50%;display:grid;place-items:center;color:white;background:var(--orange)}.ready.success .status-symbol{background:var(--green)}.ready.working .status-symbol{background:var(--blue)}.ready b{font-size:15px}.ready small{color:#697d9f;max-width:590px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.export-layout{min-height:0;display:grid;grid-template-columns:minmax(620px,1.58fr) minmax(450px,1fr);gap:14px}.export-left{min-height:0;display:grid;grid-template-rows:minmax(360px,1fr) 252px;gap:12px}.full-preview{overflow:hidden;display:flex;flex-direction:column}.preview-toolbar{min-height:58px;padding:8px 14px;border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;gap:12px}.preview-toolbar>div{min-width:0;display:flex;flex-direction:column;gap:3px}.preview-toolbar b{font-size:16px}.preview-toolbar span,.panel-head>div>span{color:var(--muted);font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.preview-toolbar .audio-inspection{color:#087d51}.preview-toolbar .audio-inspection.blocked{color:#b34b19}.preview-toolbar .btn{min-height:38px;white-space:nowrap}.export-video{flex:1;position:relative;min-height:0;background:#07101f}.export-video video{width:100%;height:100%;display:block;object-fit:contain}.export-video>span{position:absolute;top:12px;left:13px;color:#fff;background:rgba(4,15,31,.79);padding:6px 10px;border-radius:5px;font-size:14px}.caption{position:absolute;left:50%;transform:translateX(-50%);bottom:16px;color:white;background:rgba(4,15,31,.76);padding:8px 15px;border-radius:5px;font-size:18px;white-space:nowrap}.empty-video{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:9px;color:#8ea0bc;background:#0b1728}.empty-video b{color:#dfe8f5;font-size:18px}.empty-video span{font-size:14px}.review-panel{overflow:hidden}.review-panel .panel-head{height:48px}.review-panel .panel-head>div{min-width:0}.review-panel .panel-head h2{font-size:17px}.review-panel .panel-head>b{color:#526b92;font-size:14px}.shot-strip{height:133px;display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;padding:8px 12px 5px}.shot-strip article{min-width:0;padding:3px;border-radius:7px;cursor:pointer}.shot-strip article.active{background:#edf5ff}.shot-strip article>div{height:82px;border-radius:6px;position:relative;border:2px solid transparent;overflow:hidden}.shot-strip article.active>div{border-color:var(--blue)}.shot-strip video{width:100%;height:100%;display:block;object-fit:cover;border-radius:4px}.shot-strip article>div>b,.shot-strip time{position:absolute;z-index:2;color:#fff;background:#06182f;padding:3px 5px;border-radius:4px;font-size:14px}.shot-strip article>div>b{top:4px;left:4px}.shot-strip time{right:4px;bottom:4px}.shot-strip article>span{display:block;margin:4px 2px 0;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.shot-strip article>small{display:block;margin:2px 2px 0;color:var(--muted);font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.shot-strip article>small.enhanced{color:#078c57;font-weight:700}.missing-thumb{position:absolute;inset:0;display:grid;place-items:center;color:#7184a3;background:#edf3fa}.empty-export{grid-column:1/-1;display:grid;place-items:center;color:var(--muted)}.final-timeline{height:70px;padding:0 14px}.timeline-meta{height:22px;display:flex;align-items:center;justify-content:space-between;color:#637797;font-size:14px}.final-track{height:34px;display:flex;position:relative;border-radius:5px;overflow:visible}.final-track i{flex:1;display:flex;align-items:center;justify-content:center;gap:3px;font-style:normal;background:#cfe2ff;border-right:2px solid white;font-size:14px}.final-track i:first-child{border-radius:5px 0 0 5px}.final-track i:last-of-type{border-radius:0 5px 5px 0;border:0}.final-track i:nth-child(2n){background:#d8f2e8}.final-track i.notReady{color:#a75416;background:#ffebc6}.marker{position:absolute;top:-4px;bottom:-5px;width:2px;background:#ff394e}.marker:before{content:"";position:absolute;top:-1px;left:-4px;border-left:5px solid transparent;border-right:5px solid transparent;border-top:8px solid #ff394e}
.export-right{min-height:0;display:grid;grid-template-rows:238px minmax(0,1fr);gap:12px}.readiness-panel{overflow:hidden}.readiness-panel .panel-head h2{display:flex;align-items:center;gap:9px}.check-big,.check{border-radius:50%;display:grid;place-items:center;color:#fff;background:var(--green)}.check-big{width:25px;height:25px}.check-big.failed,.check-grid p.failed .check{background:#f0644c}.check-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px;padding:10px 14px}.check-grid p{min-width:0;height:44px;padding:6px 9px;border:1px solid #e0e8f3;border-radius:7px;display:grid;grid-template-columns:22px minmax(0,1fr);align-items:center;gap:6px;background:#f9fbfe}.check-grid p.failed{border-color:#ffd6c9;background:#fff8f4;cursor:pointer;transition:.15s ease}.check-grid p.failed:hover,.check-grid p.failed:focus-visible{border-color:#f0a088;background:#fff1eb;outline:0;box-shadow:0 0 0 2px rgba(240,100,76,.1)}.check{width:20px;height:20px}.check-grid p>span:last-child{min-width:0;display:flex;flex-direction:column}.check-grid p b{font-size:14px}.check-grid p small{color:#6b7f9f;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.check-grid p small em{margin-left:6px;color:#df573f;font-style:normal;font-weight:700}.export-facts{height:59px;margin:0 14px;padding:6px 10px;border-top:1px solid #e5ebf4;display:grid;grid-template-columns:1fr 1fr 1fr auto;align-items:center;gap:8px}.export-facts article{display:flex;align-items:center;gap:7px;min-width:0;color:var(--blue)}.export-facts article span{display:flex;flex-direction:column;color:#667b9c;font-size:14px}.export-facts article b{color:var(--text);font-size:14px;white-space:nowrap}.capability-badge{font-style:normal;font-size:14px;color:#c34f15;background:#fff0e7;padding:5px 7px;border-radius:5px;white-space:nowrap}.capability-badge.available{color:#078c57;background:#e8f8f1}
.export-settings{min-height:0;display:flex;flex-direction:column;padding-bottom:0;overflow-y:auto;scrollbar-gutter:stable}.export-settings .panel-head{height:52px;flex:0 0 52px}.export-settings .panel-head>div{min-width:0}.export-settings .panel-head h2{font-size:17px}.export-settings .panel-head .btn{min-height:32px}.settings-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px 10px;padding:10px 14px}.settings-grid label{display:flex;flex-direction:column;gap:4px;min-width:0}.settings-grid label>span{height:17px;display:flex;align-items:center;gap:5px;color:#405879;font-size:14px}.settings-grid select,.settings-grid input{height:32px;min-width:0;padding:0 9px;border:1px solid #d2deee;border-radius:6px;background:#fff;font-size:14px;color:var(--text)}.settings-grid .path-row{grid-column:1/-1}.path-row>div{display:grid;grid-template-columns:minmax(0,1fr) 54px;gap:5px}.path-row button{height:32px;border:1px solid #d2deee;border-radius:6px;background:#f3f6fa;color:#526b92}.mix-section{margin:0 14px;padding:8px 10px;border-radius:8px;background:#f6f9fd}.mix-title{height:30px;display:flex;justify-content:space-between;align-items:flex-start}.mix-title h3{font-size:14px}.mix-title span{display:block;color:#6b7f9f;font-size:14px}.mix-title button{border:0;background:transparent;color:var(--blue);font-size:14px}.mix-row{height:30px;display:grid;grid-template-columns:20px 43px minmax(65px,1fr) 37px 24px 76px;align-items:center;gap:5px}.mix-icon{font-size:18px;color:#274b80}.mix-row b,.mix-row span,.mix-row label{font-size:14px}.mix-row input[type=range]{min-width:0;accent-color:var(--blue)}.mix-row label{white-space:nowrap}.mute-button{border:0;background:transparent;display:grid;place-items:center;color:#37537d}.export-action{position:sticky;z-index:2;bottom:0;margin-top:auto;padding:10px 14px 12px;display:grid;grid-template-columns:minmax(0,1fr) 150px;gap:10px;align-items:stretch;border-top:1px solid #e3eaf4;background:var(--panel)}.export-state{min-width:0;padding:8px 10px;border-radius:7px;background:#fff1e9;color:#8f3f18;display:flex;flex-direction:column;justify-content:center;gap:2px}.export-state.success{background:#e8f8f1;color:#087d51}.export-state.working{background:#eaf3ff;color:#0a5fd5}.export-state span{font-size:14px;line-height:1.3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.export-btn{min-height:48px;font-size:16px}.export-btn:disabled{opacity:.65;cursor:wait}
