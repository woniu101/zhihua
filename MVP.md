...
- 优云智算 Provider 的 GPU/无卡启停、平台定时关机和状态等待已扩展为按实例执行。桌面端新增通用 worker 准备器：先确认实例已纳入管理且存在独立 SSH 配置，再处理无卡到 GPU 的关停切换，为该实例设置平台硬停止保障，等待 GPU 与 ComfyUI 就绪并写回单实例状态；SSH 未配置时不会启动 GPU，定时关机设置失败或服务 4 分钟未就绪时会请求关闭该实例。实例中心可直接准备主实例、弹性实例或测试实例，现有生成入口也复用同一准备链路。下一步把批量计算池计划接到多个 worker 的并行准备和任务分配。
