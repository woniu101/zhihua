...
10. **部分完成**：实例角色、所有权、操作与租约表已落库，启动时会分页同步并对账；Provider 已接入可用区、合法规格库存、报价、创建、精确查询和释放接口；实例中心能区分主实例、用户实例和知画受管实例，只有“知画创建 + 弹性/测试角色 + 平台确认关机 + 无任务/租约 + 无未知操作”的实例才允许确认释放。实际任务提交、轮询、取消和客户端重启恢复已经接入主 worker 租约：任务期间实例标记为忙碌，终态释放，轮询续租，过期租约在启动时回收并依据本地 worker 分配重新领取。下一步完成扩容确认单、每实例独立连接、部分创建成功、排空与弹性清理编排，再进行 1/2 worker 弹性分发、自动关机与安全释放验收，之后继续 30～90 秒多镜头成片。
