...
| 图片能力 | 公共库中的 Qwen Image 2512、Qwen Image Edit 2511 及所需编码器/VAE 已完成真实文生图与图片编辑推理；现纳入分镜关键帧 MVP，客户端、服务工作流和所见即所得画幅链路仍待实现 |
...
- 本地 FFmpeg 已真实完成 1920×1080 H.264/AAC 规范化、镜头拼接、旁白混音、可选背景音乐循环与淡入淡出、字幕烧录和 SRT 输出；当前导出实现仍强制依赖 1080p 衍生文件，必须改为正式候选可直接导出并增加候选清晰度选项。H3 环境音保持禁用，直到试听与保留流程完成 GPU 验证。
...
1. 先纠正本地版本与导出语义：正式候选原片可直接进入时间线和导出；1080p 改为可选衍生文件，导出页提供候选清晰度与 1080p 两种路径。
2. 建立 `FrameComposition`、图片版本和清晰度衍生数据模型及 SQLite 迁移，保留现有项目兼容性。
3. 实现统一画幅组件：项目比例选择、上传/粘贴图片适配、非破坏性裁切/完整显示、主体位置调整、安全区预览和 H3 工作图派生。
4. 修改 H3 工作流，在视频编码前按同一安全区输出严格目标比例的候选文件；SeedVR2 直接使用该候选，不再次裁切。
5. 接入 `ComfyUIQwenImageProvider`、两条固定图片工作流及分镜内的生成/编辑入口；生成图片和上传图片复用同一画幅组件与素材版本管理。
6. 完成无 GPU 的数据库、任务恢复、导出、UI 和 Tauri 完整构建验证，并生成嵌入前端的 release EXE。
7. 再开启 5090，实测 16:9、9:16、1:1 的图片生成/编辑 → H3 候选 → 候选导出链路，以及可选 SeedVR2；记录 5/10/15 秒耗时、显存和费用。
8. 最后完成 30～90 秒多镜头、故障恢复和弹性多 worker 验收。
