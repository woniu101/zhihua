...
- 图片生成、图片编辑、H3 候选和 SeedVR2 增强已统一为“准备环境 → 预留就绪 worker → 向该实例上传全部输入 → 使用同一租约提交”的顺序。预留采用 SQLite 即时事务，多个并发请求不会领取同一空闲实例；上传或提交失败时，客户端按明确的实例 ID 删除受控输入并释放预留，任务进入远端后继续沿用原 worker 做轮询、取消和下载。当前主实例已接入该路径，弹性实例仍需自动准备环境后才能进入可选 worker 集合。
