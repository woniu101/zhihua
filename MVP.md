...
- 图片素材版本已接入独立 `FrameComposition`：按“素材版本 × 项目画幅”保存填满或完整显示、主体位置、留白方式、工作尺寸、可见尺寸与 H3 安全裁切坐标。素材页可在最终画框中预览并保存构图，原图不被覆盖；FFmpeg 画幅派生文件和 H3 上传替换仍待接入。
...
- 修复桌面端对服务端 snake_case 结果清单的解析；原生 Rust 客户端已通过真实服务的兼容握手、9 条工作流能力读取、已完成任务恢复和结果清单反序列化。
...
2. **已完成基础结构**：前后端集中定义五种 `FrameProfile`，候选版本在独立关系表保存生成画幅、H3 工作尺寸、最终可见尺寸和裁切坐标；图片版本已接入 `FrameComposition`，清晰度衍生关系仍待扩展。
3. **部分完成**：项目画幅已按项目保存并可在分镜页选择；候选画幅不一致时不能设为正式版本或导出。素材页已提供上传/粘贴图片的非破坏性填满、完整显示、主体位置和安全画框预览；实际画幅派生文件、边缘延展与分镜生成输入替换仍待实现。
4. **已完成**：H3 八条工作流已在编码前加入精确可见画框裁切，SeedVR2 直接处理候选帧、不再二次裁切；`h3-workflows-2026.09.11` 已在 16:9、9:16、1:1 和 1920×1080 增强链路上完成 5090 实测。
5. **当前实施项**：接入 `ComfyUIQwenImageProvider`、两条固定图片工作流及分镜内的生成/编辑入口；生成图片和上传图片复用同一 `FrameComposition`、原图版本和画幅派生管理。
...
8. 图片链路完成后再开启 5090 验证 Qwen Image 生成与编辑的精确画幅，最后完成 30～90 秒多镜头、故障恢复和弹性多 worker 验收。
