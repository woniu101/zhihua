...
<template>
  <section class="page settings-page">
    ...
    <div class="settings-grid">
      <section class="panel account"><div class="panel-head"><h2>账户概览</h2><span>最近实测快照 · 2026-09-10　<RefreshCw :size="17"/></span></div><div class="money-row"><article><Database :size="34"/><div><span>可用余额</span><b>¥ 13.39</b></div></article><article><span>当前无卡费率</span><b>--</b><small>平台未返回费率</small></article><article class="cost"><Calculator :size="32"/><div><span>本次运行估算</span><b>暂无法估算</b><small>优云智算账户接口接入后实时计算</small></div></article></div><div class="account-actions"><button class="btn primary" disabled title="优云智算账户接口接入后开放"><Wallet :size="19"/>充值</button><button class="btn" disabled title="优云智算账单接口接入后开放"><FileTextIcon/>查看平台账单</button></div></section>

      <section class="panel instance"><div class="panel-head"><h2>开发实例快照</h2><span :class="connected ? 'success-text' : 'waiting-text'"><span class="dot" :class="{ gray: !connected }"></span>{{ connected ? '维护服务正常' : '等待本机隧道' }}</span></div><div class="instance-main"><div class="server-art">▤</div><div><h2>⌖ 乌兰察布一区</h2><p>RTX 5090 32GB</p><span class="muted"><span class="dot gray"></span>无卡运行 · 2 核 4GB</span></div><span class="muted"><span class="dot gray"></span>{{ probe?.comfyuiReady ? '可生成' : '生成服务待启动' }}</span></div><div class="instance-metrics"><div><span>当前模式</span><b>无卡运行</b></div><div><span>GPU 规格</span><b>RTX 5090 32GB</b></div><div><span>区域/可用区</span><b>cn-wlcb-01</b></div></div><div class="instance-actions"><button class="btn primary" disabled title="优云智算实例接口接入后开放"><Power :size="18"/>启动 GPU</button><button class="btn" disabled><Wrench :size="18"/>无卡维护中</button><button class="btn" disabled title="控制台链接等待实例接口返回"><ExternalLink :size="17"/>打开优云智算控制台</button></div></section>
