...
import { CalendarClock, Calculator, Database, ExternalLink, FolderOpen, Gauge, HardDrive, KeyRound, Laptop, Monitor, Moon, Palette, Power, RefreshCw, Server, ShieldCheck, Sun, Timer, Wallet, Wrench, X } from "lucide-vue-next";
import {
...
onMounted(() => Promise.allSettled([refreshConnection(), refreshCompute(), refreshDeepSeek(), refreshStorage()]));
</script>
...
<template>
  <section class="page settings-page">
    ...
    <nav class="settings-tabs"><button type="button" @click="activeSection='compute'; deepSeekOpen=true"><KeyRound :size="22"/>连接状态</button><button :class="{ active: activeSection === 'compute' }" type="button" @click="activeSection='compute'"><Server :size="22"/>算力实例</button><button type="button" title="环境状态显示在算力页中" @click="activeSection='compute'"><ShieldCheck :size="22"/>环境检查</button><button type="button" disabled title="将在语音与导出阶段开放"><Gauge :size="22"/>语音与导出</button><button :class="{ active: activeSection === 'appearance' }" type="button" @click="activeSection='appearance'"><Monitor :size="22"/>存储与外观</button></nav>
