...
10. **部分完成**：实例角色、所有权、操作与租约表已落库，启动时会分页同步并对账；Provider 已接入可用区、合法规格库存、报价、创建、精确查询和释放接口；实例中心能区分主实例、用户实例和知画受管实例，只有“知画创建 + 弹性/测试角色 + 平台确认关机 + 无任务/租约 + 无未知操作”的实例才允许确认释放。实际任务提交、轮询、取消和客户端重启恢复已经接入主 worker 租约：任务期间实例标记为忙碌，终态释放，轮询续租，过期租约在启动时回收并依据本地 worker 分配重新领取。扩容确认单已接入实例中心：以主实例生成精确规格，先查询实时库存与按量报价，规格变化后强制重新预检，用户再次确认才以稳定幂等键创建一台弹性实例；创建结果不确定时停止重复提交并进入对账。每个实例的服务就绪状态现已独立落库，记录连接中、等待 GPU、就绪、版本不兼容和不可达，以及 API、服务、工作流和模型清单版本；主实例通过 SSH 握手时会更新自身记录，切换实例后不会沿用上一实例的服务令牌。下一步把单一隧道管理器扩展为按实例隔离的连接池，并完成多台创建的部分成功处理、排空与弹性清理编排，再进行 1/2 worker 弹性分发、自动关机与安全释放验收，之后继续 30～90 秒多镜头成片。
