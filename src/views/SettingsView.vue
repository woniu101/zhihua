...
const activeSection = ref<"overview" | "workers" | "connection" | "storage">("overview");
...
<template>
  <section class="page settings-page">
    ...
    <nav class="settings-tabs">
      <button :class="{ active: activeSection === 'overview' }" type="button" @click="activeSection='overview'"><Server :size="20"/>算力概览</button>
      <button :class="{ active: activeSection === 'workers' }" type="button" @click="activeSection='workers'"><Database :size="20"/>实例池 <i v-if="managedInstances.length">{{ managedInstances.length }}</i></button>
    ...
    <div v-if="activeSection === 'overview'" class="settings-grid settings-overview">
    ...
    <div v-else-if="activeSection === 'workers'" class="worker-page">
      <section class="worker-summary">
        <article><span>实例总数</span><b>{{ managedInstances.length }}</b><small>主实例与弹性实例统一管理</small></article>
      ...
      <section class="panel worker-pool-panel">
        <div class="panel-head worker-pool-head"><div><h2>实例池</h2><p>每台实例独立连接、领取任务和执行关机保护；资源紧张时可随时扩容。</p></div><div><button class="btn" type="button" :disabled="busy" @click="refreshCompute"><RefreshCw :size="16"/>刷新</button><button class="btn primary" type="button" :disabled="busy || !elasticTemplateAvailable" @click="openElasticCreate"><Plus :size="16"/>新增弹性实例</button></div></div>
        ...
        <footer class="worker-pool-foot"><ShieldCheck :size="20"/><span><b>费用保护对每台实例独立生效</b><small>客户端退出、任务异常和平台对账均不会把“未知状态”当作已关机。</small></span><button class="btn" type="button" @click="activeSection='overview'">调整保持策略</button></footer>
      </section>
    </div>
...
.settings-tabs{gap:18px}.settings-tabs button>i{min-width:20px;height:20px;padding:0 6px;border-radius:10px;display:grid;place-items:center;background:var(--blue-soft);color:var(--blue);font-size:13px;font-style:normal}.settings-overview{grid-template-rows:294px minmax(0,1fr)}.settings-overview .shutdown{grid-column:1/2}.overview-retention{grid-column:2/4;position:relative;padding-bottom:62px}.overview-retention .worker-link{position:absolute;right:18px;bottom:14px;min-height:36px;font-size:13px}.worker-page{min-height:0;padding-top:14px;display:grid;grid-template-rows:98px minmax(0,1fr);gap:13px}.worker-summary{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.worker-summary article{padding:15px 18px;border:1px solid var(--line);border-radius:10px;background:var(--panel);box-shadow:var(--shadow);display:grid;grid-template-columns:1fr auto;gap:5px 12px;align-items:center}.worker-summary span{color:var(--muted);font-size:13px}.worker-summary b{grid-row:1/3;grid-column:2;font-size:29px;color:var(--navy)}.worker-summary small{font-size:13px}.worker-pool-panel{min-height:0;display:flex;flex-direction:column;overflow:hidden}.worker-pool-head{min-height:68px;padding-block:9px}.worker-pool-head>div:first-child{min-width:0}.worker-pool-head p,.worker-health-panel .panel-head p{margin-top:5px;color:var(--muted);font-size:14px}.worker-pool-head>div:last-child{display:flex;gap:8px}.worker-pool-head .btn{min-height:38px;font-size:13px}.worker-notice{margin:10px 16px 0;padding:9px 12px;border-radius:7px;background:var(--surface-soft);color:var(--muted);font-size:14px}.worker-notice.success{background:#e8f8f1;color:#087d51}.worker-notice.error{background:#fff0ee;color:#b33b35}.worker-page-list{flex:1;min-height:0;max-height:none;padding:12px 16px;overflow:auto}.worker-page-list .instance-row-main{grid-template-columns:82px minmax(0,1fr) 120px}.worker-page-list .instance-row-main>strong{display:flex;flex-direction:column;align-items:flex-end;gap:4px}.worker-page-list .instance-row-main>strong small{margin:0;font-size:13px;font-weight:400}.worker-pool-foot{min-height:64px;padding:9px 16px;border-top:1px solid var(--line);display:grid;grid-template-columns:28px minmax(0,1fr) auto;align-items:center;gap:10px}.worker-pool-foot>svg{color:var(--green)}.worker-pool-foot>span{display:flex;flex-direction:column;gap:4px}.worker-pool-foot small{color:var(--muted);font-size:13px}.worker-pool-foot .btn{min-height:36px;font-size:14px}.connection-page{min-height:0;padding-top:14px;display:grid;grid-template-columns:minmax(500px,1fr) minmax(500px,1fr);grid-template-rows:minmax(0,1fr) 150px;gap:13px}.connection-page>.environment,.worker-health-panel{min-height:0;overflow:hidden}.connection-full{grid-column:1/3}.worker-health-panel{display:flex;flex-direction:column}.worker-health-panel>.panel-head{min-height:62px}.worker-health-panel>.panel-head>div{min-width:0}.worker-health-list{padding:8px 14px;overflow:auto}.worker-health-list article{min-height:62px;padding:7px 10px;border-bottom:1px solid var(--line);display:grid;grid-template-columns:10px minmax(0,1fr) 100px 62px;align-items:center;gap:10px}.worker-health-list article>div{min-width:0;display:flex;flex-direction:column;gap:4px}.worker-health-list article b,.worker-health-list article small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.worker-health-list article strong{text-align:right;font-size:14px}.worker-health-list article strong.ready{color:var(--green)}.worker-health-list article strong.error{color:#c4423a}.storage-actions{margin:12px 18px 0;display:grid;grid-template-columns:1fr 120px;gap:9px}.storage-actions .btn{min-height:42px;font-size:13px}.storage-path-card.pending{border-color:#9bc1f5;background:var(--blue-soft)}.storage-path-card.pending.unavailable{border-color:#efb272;background:#fff7ea}.storage-path-card.pending.unavailable>span{color:#d9660a;background:#fff0db}.storage-path-card.database-card{min-height:76px}.local-storage-panel .open-storage{margin-top:10px;min-height:40px;height:40px}.local-storage-panel:has(.storage-path-card.pending) .storage-path-card{min-height:74px;margin-top:9px}.local-storage-panel:has(.storage-path-card.pending) .storage-meta{height:42px;margin-top:9px}
...
.overview-metrics>.panel-head>div:first-child p{margin-top:4px;color:var(--muted);font-size:13px}
...
.overview-stat-grid small{color:var(--muted);font-size:13px;font-weight:700}
