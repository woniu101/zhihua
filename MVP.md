...
用户在知画内确认地域、单卡配置、按量报价与关机后的存储费用后，才能引导创建实例。创建请求结果不明时先查询资源，禁止盲目重试导致重复购买。日常启动遵循用户已选择的实例与设置；不得因缺货静默更换地域、增加 GPU、扩容或创建第二台实例。
...
`GpuInstance` 必须保留地域、可用区、实际运行模式、原 GPU 规格、最近启动与关机时间以及可选回收时间。价格与余额类型必须携带币种、单位、查询时间和可空值；保留期信息区分平台确认值与估计值。`create` 仅在用户确认配置后调用，不能由普通 `start` 或缺货重试隐式触发。云端模型清单、链接和固定工作流准备由独立的环境准备模块负责。
...
- 商业视频 API 的真实接入。
- AutoDL 等第二云平台的真实接入。
- 未经用户确认的实例购买，以及自动释放、扩容、跨区迁移或缺货时自动新建替代实例。
...
- [ ] 可以查询兼容配置、模型区域、库存和分项报价，用户确认后只创建一台实例；超时恢复不重复购买。
...
| 服务边界 | 测试通过 ComfyUI API 完成，但尚未部署知画云端服务，也未完成客户端到 30～90 秒多分镜成片的端到端验收 |
