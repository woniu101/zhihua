...
import { AlertTriangle, BrainCircuit, Check, ChevronRight, Clock3, FileText, Pencil, Plus, Sparkles, Trash2, Upload, X } from "lucide-vue-next";
...
const contentStage = computed(() => !store.items.value.length ? 1 : !store.points.value.length || store.unresolvedCount.value > 0 ? 2 : 3);
const contentSteps = [
  { index: 1, label: "资料", hint: "导入或输入主题" },
  { index: 2, label: "知识点", hint: "核对事实与来源" },
  { index: 3, label: "分镜计划", hint: "确认后生成镜头" },
];
...
<template>
  <section class="page sources-page" :class="{ empty: !store.items.value.length && !store.points.value.length }" @dragenter.prevent="dragging=true" @dragover.prevent @dragleave.self="dragging=false" @drop.prevent="onDrop">
    <header class="page-head">
      <div class="content-title"><div class="page-title-line"><h1>内容</h1><span class="subtitle">先确认内容，再消耗 GPU 生成画面</span></div><ol class="content-steps"><li v-for="step in contentSteps" :key="step.index" :class="{active:contentStage===step.index,done:contentStage>step.index}"><b>{{ contentStage>step.index ? '✓' : step.index }}</b><span>{{ step.label }}<small>{{ step.hint }}</small></span></li></ol></div>
      <div class="head-actions"><ComputeStatus context="内容整理不使用 GPU" local-only/><button class="btn" @click="chooseFiles"><Upload :size="19"/>导入资料</button><button class="btn" @click="pasteDialog=true"><FileText :size="18"/>粘贴文字</button></div>
    ...
    <div class="sources-main">
      ...
      <section class="panel document-panel">
        ...
        <div v-else class="no-preview full"><Upload :size="44"/><strong>导入第一份资料</strong><span>支持 PDF、PPTX、DOCX、TXT 和常见图片，也可以直接粘贴一段文字。</span><div class="empty-import-actions"><button class="btn primary" @click="chooseFiles"><Upload :size="17"/>选择文件</button><button class="btn" @click="pasteDialog=true"><FileText :size="17"/>粘贴文字</button></div></div>
      </section>

      <aside class="panel insight-panel">
        <div class="ai-head"><span class="planner-icon"><BrainCircuit :size="23"/></span><span class="dot"></span><div><strong>大模型内容规划</strong><small>{{ store.enabledReadyCount.value }} 份资料可用于提取</small></div><button :disabled="!activeSources.length" @click="extractKnowledge">{{ store.points.value.length ? '重新提取' : '开始提取' }}</button></div>
...
<style scoped>
.sources-page{display:grid;grid-template-rows:80px auto minmax(0,1fr) 254px;gap:10px}.sources-page:not(:has(.notice)){grid-template-rows:80px minmax(0,1fr) 254px}.compute{height:48px;padding:0 14px;border:1px solid var(--line);border-radius:9px;background:#fff}.hidden-input{display:none}.notice{min-height:34px;margin:0 2px;padding:7px 10px;border:1px solid #ffd08a;border-radius:7px;display:flex;align-items:center;gap:7px;background:#fff7e9;color:#9b5a10;font-size:14px}.notice button{margin-left:auto;border:0;background:transparent;color:inherit;font-size:19px}.sources-main{min-height:0;display:grid;grid-template-columns:350px minmax(440px,1fr) 396px;gap:12px}.file-panel,.document-panel,.insight-panel{min-height:0;overflow:hidden}.file-tabs{display:flex;gap:5px;padding:9px 10px;overflow-x:auto}.file-tabs button{height:33px;border:0;border-radius:6px;padding:0 10px;white-space:nowrap;background:#f1f5fb;color:#53688e}.file-tabs .active{color:white;background:var(--blue)}.file-list{height:calc(100% - 90px);padding:0 8px;overflow-y:auto}.file-list article{position:relative;min-height:88px;border-bottom:1px solid #e9eef6;display:grid;grid-template-columns:46px 1fr 32px;align-items:center;gap:9px;padding:7px 8px;cursor:pointer}.file-list article.selected{border:2px solid var(--blue);border-radius:9px;background:#f4f8ff}.file-list article.disabled{opacity:.58}.file-icon{width:31px;height:40px;border-radius:4px;display:grid;place-items:center;color:#fff;font-weight:800;font-size:14px;flex:0 0 auto}.file-icon.red{background:#e63746}.file-icon.orange{background:#f06c23}.file-icon.blue{background:#2874da}.file-icon.slate{background:#667995}.file-list article>div:nth-child(2){display:flex;flex-direction:column;gap:4px;min-width:0}.file-list strong{font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.file-list span,.file-list em{font-size:14px;color:#687da0}.file-list em{font-style:normal}.file-list .ready{color:#0a9d62}.file-list .working{color:#2c72d7}.file-list .error{color:#d84850}.file-actions{display:flex;flex-direction:column;gap:7px;align-items:center}.toggle{width:34px;height:19px;border:0;border-radius:20px;background:#ced8e7;position:relative}.toggle:after{content:"";position:absolute;width:15px;height:15px;left:2px;top:2px;border-radius:50%;background:#fff}.toggle.on{background:var(--blue)}.toggle.on:after{left:17px}.delete{width:27px;height:25px;border:0;border-radius:5px;display:grid;place-items:center;background:transparent;color:#9aabc3}.delete:hover{background:#fff0f0;color:#dd3d46}.mini-progress{height:3px;border-radius:3px;background:#dbe6f5;overflow:hidden}.mini-progress b{display:block;height:100%;background:var(--blue)}.empty-list{padding:40px 12px;text-align:center;color:#8394af}.doc-toolbar{height:54px;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:11px;padding:0 14px;color:#51678c}.doc-toolbar .file-icon{width:25px;height:31px}.doc-toolbar strong{font-size:15px;color:#1a315b;max-width:42%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.divider{height:24px;width:1px;background:#dfe6f1;margin:0 5px}.plain-action{margin-left:auto;border:0;background:transparent;color:var(--blue);display:flex;align-items:center;gap:5px}.paper{height:calc(100% - 54px);padding:30px 36px;overflow-y:auto;background:linear-gradient(90deg,#fff,#fcfdff);user-select:text}.paper.pending{background:#fafcff}.paper h2{font-size:23px;margin-bottom:20px}.paper p{font-size:17px;line-height:1.85;margin-bottom:19px;color:#213c6a;white-space:pre-wrap}.paper .highlight{background:#dceeff;box-shadow:0 0 0 4px #dceeff;border-radius:2px}.no-preview{height:80%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:9px;color:#7a8dac}.no-preview strong{font-size:18px;color:#2c4771}.no-preview.full{height:100%}.insight-panel{display:flex;flex-direction:column}.ai-head{min-height:66px;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:9px;padding:0 14px}.deepseek{font-size:30px;color:#1769ef}.ai-head>div{display:flex;flex-direction:column;gap:3px}.ai-head small{color:#7385a4}.ai-head>button{margin-left:auto;border:1px solid #d4dfef;border-radius:7px;background:#fff;height:34px}.insight-scroll{flex:1;min-height:0;overflow:auto;padding:14px}.two-fields{display:grid;grid-template-columns:1fr 1fr;gap:12px}.two-fields label{font-size:14px;font-weight:700}.two-fields span{height:45px;margin-top:6px;border:1px solid #d4dfef;border-radius:7px;display:flex;align-items:center;gap:6px;padding:0 10px;font-weight:600}.subhead{display:flex;align-items:center;justify-content:space-between;margin:15px 0 8px}.subhead button{height:31px;border:1px solid #d5e1f2;border-radius:6px;background:white;color:var(--blue)}.subhead>span{color:#7184a4;font-size:14px}.point-list{display:flex;flex-direction:column;gap:6px}.point-list>button{min-height:40px;border:1px solid #e0e7f1;border-radius:7px;display:grid;grid-template-columns:26px 1fr 22px;align-items:center;padding:5px 9px;background:#fff;text-align:left;font-size:14px}.point-list b{width:22px;height:22px;border-radius:50%;display:grid;place-items:center;background:#eaf3ff;color:var(--blue)}.point-list i{font-style:normal;color:#6680a5}.point-list .warn{border-color:#ffbf82;background:#fff7ed;color:#d95d00}.citation{font-size:14px;padding:7px 9px;color:#5b6f92;cursor:pointer}.citation.active{border-left:3px solid var(--blue);color:var(--blue);background:#f0f6ff}.generate{margin:8px 14px 4px;min-height:50px;font-size:16px}.generate:disabled{cursor:not-allowed;opacity:.48}.confirm-hint{text-align:center;margin:2px 0 8px;color:#d46a16;font-size:14px}.knowledge-panel{overflow:hidden}.knowledge-panel>.panel-head>span{font-size:14px;color:#7184a4}.knowledge-grid{height:calc(100% - 46px);display:grid;grid-auto-flow:column;grid-auto-columns:minmax(220px,1fr);gap:8px;padding:8px 10px 12px;overflow-x:auto}.knowledge-grid article{height:185px;border:1px solid #d8e3f2;border-radius:8px;padding:10px;display:grid;grid-template-columns:27px 1fr;grid-template-rows:29px 1fr 35px;gap:5px}.knowledge-grid article>b{width:24px;height:24px;border-radius:8px;display:grid;place-items:center;background:#eaf3ff;color:var(--blue)}.knowledge-grid article>input{min-width:0;border:0;border-bottom:1px solid transparent;outline:none;font-weight:700;color:#203a65;background:transparent}.knowledge-grid article>input:focus{border-color:var(--blue)}.knowledge-grid article textarea{grid-column:1/3;resize:none;border:1px solid transparent;border-radius:5px;padding:5px;background:transparent;color:#617697;font-size:14px;line-height:1.45}.knowledge-grid article textarea:focus{outline:none;border-color:#b8d4ff;background:#fff}.knowledge-grid article footer{grid-column:1/3;display:flex;flex-direction:column;gap:3px;font-size:14px;color:#6880a3;overflow:hidden}.knowledge-grid article footer span{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.knowledge-grid article footer label{display:flex;align-items:center;gap:4px;color:#24724d}.knowledge-grid article.warn{border-color:#ffae6e;background:#fffaf4}.add-point{height:185px;border:1px dashed #a8bcd9;border-radius:8px;background:#f8fbff;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;color:var(--blue)}.small{min-height:32px;padding:0 10px}.drop-overlay{position:fixed;z-index:90;inset:60px 24px 24px;border:3px dashed #58a0ff;border-radius:18px;background:rgba(239,247,255,.94);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;color:#1b6de3}.drop-overlay strong{font-size:24px}.modal-backdrop{position:fixed;z-index:100;inset:0;background:rgba(8,25,55,.38);display:grid;place-items:center}.paste-dialog{width:560px;padding:22px;border:1px solid #d6e1ef;border-radius:14px;background:#fff;box-shadow:0 22px 60px rgba(14,40,78,.25)}.paste-dialog header{display:flex;justify-content:space-between;margin-bottom:16px}.paste-dialog header p{margin-top:6px;color:#7184a4;font-size:14px}.paste-dialog header button{width:34px;height:34px;border:0;border-radius:7px;background:#eef3f9}.paste-dialog label{display:flex;flex-direction:column;gap:7px;margin-top:12px;font-size:14px;font-weight:700}.paste-dialog input,.paste-dialog textarea{border:1px solid #cddbef;border-radius:8px;padding:10px;outline:none}.paste-dialog input{height:43px}.paste-dialog textarea{height:220px;resize:vertical;line-height:1.6}.paste-dialog input:focus,.paste-dialog textarea:focus{border-color:var(--blue);box-shadow:0 0 0 3px #e5f0ff}.paste-dialog footer{display:flex;justify-content:flex-end;gap:10px;margin-top:18px}@media(max-width:1380px){.sources-main{grid-template-columns:300px minmax(390px,1fr) 350px}.paper{padding:25px}.paper p{font-size:15px}.knowledge-panel{height:220px}.sources-page,.sources-page:not(:has(.notice)){grid-template-rows:74px minmax(0,1fr) 220px}.sources-page:has(.notice){grid-template-rows:74px auto minmax(0,1fr) 220px}.head-actions{gap:6px}.head-actions .btn{padding:0 9px}}
.sources-page.empty{grid-template-rows:80px auto minmax(0,1fr) 155px}.sources-page.empty:not(:has(.notice)){grid-template-rows:80px minmax(0,1fr) 155px}.empty-import-actions{display:flex;gap:9px;margin-top:6px}.empty-import-actions .btn{min-height:42px}.planner-icon{width:38px;height:38px;border-radius:10px;display:grid;place-items:center;background:#eaf3ff;color:#1769ef;flex:0 0 auto}.ai-head>button:disabled{opacity:.45;cursor:not-allowed}.knowledge-empty{height:calc(100% - 46px);padding:10px 18px;display:flex;align-items:center;justify-content:space-between;gap:24px;background:#fbfdff}.knowledge-empty ol{display:flex;align-items:center;gap:44px;margin:0;padding:0;list-style:none}.knowledge-empty li{display:flex;align-items:center;gap:10px}.knowledge-empty li>b{width:30px;height:30px;border-radius:50%;display:grid;place-items:center;background:#e8f2ff;color:var(--blue)}.knowledge-empty li>span{font-weight:700;color:#1d3a67}.knowledge-empty small{display:block;margin-top:3px;color:#7386a5;font-size:14px;font-weight:400}@media(max-width:1380px){.sources-page.empty,.sources-page.empty:not(:has(.notice)){grid-template-rows:74px minmax(0,1fr) 145px}.sources-page.empty:has(.notice){grid-template-rows:74px auto minmax(0,1fr) 145px}.knowledge-empty ol{gap:20px}}
.content-title{display:flex;align-items:center;gap:24px}.content-steps{display:flex;align-items:center;gap:0;margin:0;padding:0;list-style:none}.content-steps li{position:relative;min-width:120px;padding-left:38px;display:flex;align-items:center;color:#778aa8}.content-steps li:not(:last-child):after{content:"";position:absolute;top:16px;left:99px;width:38px;height:1px;background:#d6e0ed}.content-steps li>b{position:absolute;left:0;width:29px;height:29px;border-radius:50%;display:grid;place-items:center;background:#eef3f9;color:#7185a5;font-size:14px}.content-steps li>span{font-size:14px;font-weight:700;white-space:nowrap}.content-steps small{display:block;margin-top:2px;color:#8998ae;font-size:14px;font-weight:400}.content-steps li.active{color:#0869ef}.content-steps li.active>b{color:#fff;background:var(--blue);box-shadow:0 3px 8px rgba(15,111,255,.2)}.content-steps li.done>b{color:#08784f;background:#e3f6ed}.content-steps li.done:after{background:#87cbb0}@media(max-width:1450px){.content-steps li{min-width:100px}.content-steps li:not(:last-child):after{left:91px;width:23px}.content-title{gap:15px}.content-steps small{display:none}}
</style>
