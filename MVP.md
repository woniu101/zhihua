...
10. **当前实施项**：先完成不依赖 GPU 的实例控制面：持久化实例角色、所有权、操作和租约，启动时分页同步并对账；随后接合法规格、库存、报价、创建、停止和释放 API，完成实例中心、扩容确认、排空及释放保护。最后启动 5090 进行单实例故障注入、1/2 worker 弹性分发、自动关机与安全释放验收，再继续 30～90 秒多镜头成片。
