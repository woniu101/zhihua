  ...
  <section class="page projects-page" @click="menuId = undefined">
    <header class="page-head">
      ...
      <div class="head-actions">
        <ComputeStatus context="需要生成时再切换 GPU"/>
    ...
    <div v-else class="empty-projects"><FolderOpen v-if="!store.items.value.length" :size="38"/><Search v-else :size="34"/><strong>{{ store.items.value.length ? '没有符合条件的项目' : '创建第一个项目' }}</strong><span>{{ store.items.value.length ? '换个关键词或筛选条件试试' : '项目、素材、候选视频和导出成片都会保存在本机。' }}</span><button v-if="!store.items.value.length" class="btn primary" @click="showCreate"><Plus :size="18"/>新建项目</button></div>
