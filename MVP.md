...
8. **当前实施项**：图片任务已进入本地持久队列，支持项目级素材任务、客户端重启后恢复、取消以及完成后继续下载；知画服务会在任务成功或确定失败后清理受控上传输入。下一步实现统一任务中心展示和真实取消 ComfyUI 执行；随后做 9:16、1:1 图片画幅抽检、30～90 秒多镜头成片、故障注入和弹性多 worker 验收。
