...
- 设置页展示已经接入的算力与连接、统一大模型 Provider、存储和主题能力。GPU 保持策略可选省费用、任务优先或持续 GPU，并显示 3/15 分钟空闲行为及 60/180/720 分钟平台硬上限；切换后立即更新正在运行实例的保护时间。尚无可靠平台接口的七天保活继续保持未启用，避免出现可点击但无效果的开关。
