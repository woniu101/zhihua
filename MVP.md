...
- 分镜页批量生成已接入计算池计划，默认“单路”。用户可在提交前明确选择 2、4 或 8 路上限；客户端始终先准备主实例，再从已有的知画弹性/测试实例中按任务数选取需要的 worker 并并行准备，不会因此自动购买新实例。某个弹性实例缺少 SSH 配置、无库存或服务未就绪时，其余已就绪 worker 继续接单，并在批量状态中显示实际并行路数、后续轮次任务数和降级数量。每个任务仍先取得独立 worker 租约，再向对应实例上传素材和提交。
