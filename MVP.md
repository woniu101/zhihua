...
2. **已完成基础结构**：前后端集中定义五种 `FrameProfile`，候选版本在独立关系表保存生成画幅、H3 工作尺寸、最终可见尺寸和裁切坐标；图片版本、`FrameComposition` 和清晰度衍生关系仍待接入。
3. **部分完成**：项目画幅已按项目保存并可在分镜页选择；候选画幅不一致时不能设为正式版本或导出。上传/粘贴图片的非破坏性裁切、完整显示、主体位置调整和安全区预览仍待实现。
4. **待 GPU 验证**：H3 八条工作流已在编码前加入精确可见画框裁切，SeedVR2 已改为直接处理候选帧、不再二次裁切；工作流清单版本提升为 `h3-workflows-2026.09.11`。
...
7. 下一步开启 5090，实测 16:9、9:16、1:1 的精确 H3 候选与可选 SeedVR2；确认裁切节点、尺寸、显存和耗时后，再接入并验证图片生成/编辑链路。
