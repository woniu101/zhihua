...
10. **当前实施项**：不依赖 GPU 的实例控制面基础已完成：实例角色、所有权、操作与租约表已落库，启动时会分页同步并对账；Provider 已接入可用区、合法规格库存、报价、创建、精确查询和释放接口；实例中心能区分主实例、用户实例和知画受管实例，只有“知画创建 + 弹性/测试角色 + 平台确认关机 + 无任务/租约 + 无未知操作”的实例才允许确认释放。下一步把 worker 租约接入实际任务调度，完成扩容确认单、部分创建成功、排空与弹性清理编排；之后启动 5090 进行单实例故障注入、1/2 worker 弹性分发、自动关机与安全释放验收，再继续 30～90 秒多镜头成片。
