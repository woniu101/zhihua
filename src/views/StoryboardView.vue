...
import { ArrowDown, ArrowUp, ChevronDown, Copy, Film, FolderOpen, Lock, LoaderCircle, Merge, Music2, Play, Plus, RotateCcw, Scissors, Sparkles, Star, Subtitles, Trash2, Unlock, X } from "lucide-vue-next";
...
type GenerationSpeed = "economy" | "auto" | "fast";
const generationSpeed = ref<GenerationSpeed>("auto");
const batchWorkerLimit = computed<1 | 2 | 8>(() => generationSpeed.value === "economy" ? 1 : generationSpeed.value === "fast" ? 8 : 2);
const generationSpeedOptions: Array<{ id: GenerationSpeed; label: string; summary: string; detail: string }> = [
  { id: "economy", label: "省钱排队", summary: "单路依次生成", detail: "最容易获得算力，启动开销少，等待时间较长。" },
  { id: "auto", label: "智能安排", summary: "推荐 · 自动并行", detail: "根据镜头和已有实例安排，速度与费用更均衡。" },
  { id: "fast", label: "加速并行", summary: "尽量同时生成", detail: "完成更快，受实例和 GPU 库存影响，启动与空闲费用可能略高。" },
];
const selectedSpeedOption = computed(() => generationSpeedOptions.find((item) => item.id === generationSpeed.value)!);
const pendingBatchCount = computed(() => scenes.value.filter((scene) =>
  !scene.locked && !["generated", "approved", "generating"].includes(scene.status)
).length);
const plannedParallelLanes = computed(() => Math.min(batchWorkerLimit.value, Math.max(1, pendingBatchCount.value)));
...
async function prepareBatchWorkerPool(taskCount: number) {
  ...
  if (workerLimit > 1) {
    ...
    while (preparedWorkers < workerLimit && candidateOffset < elasticCandidates.length) {
      ...
      batchNotice.value = `主实例已就绪，正在并行准备 ${candidates.length} 个生成通道…`;
...
async function generateIncompleteScenes() {
  ...
  try {
    ...
    batchNotice.value = pool.plan.plannedWorkers > 1
      ? `${pool.plan.plannedWorkers} 路生成通道已就绪，正在分配 ${valid.length} 个分镜…`
      : `单路生成已就绪，正在将 ${valid.length} 个分镜加入队列…`;
  } catch (error) {
    ...
    <div class="story-workspace">
      ...
      <section class="preview-column">
        <div class="video-card panel">
          ...
          <div v-if="!previewVersion" class="empty-player-state"><span>生成候选后可在这里播放、比较并选择正式版本</span></div>
        </div>
...
<style scoped>
.storyboard-page{display:grid;grid-template-rows:90px minmax(0,1fr) 280px;gap:12px}.story-head{display:flex;align-items:center;justify-content:space-between;padding:0 4px}.breadcrumb{color:#54698e;margin-bottom:7px}.format-pill{height:49px;padding:0 14px;border:1px solid var(--line);border-radius:9px;background:#fff;display:flex;align-items:center;gap:8px;font-size:14px}.format-pill b{font-size:15px}.format-pill select{border:1px solid #ccd9ea;border-radius:6px;background:#f7faff;color:#12335f;font-weight:750;padding:5px 24px 5px 8px}.format-pill i{height:20px;width:1px;background:#dce4ef}.story-workspace{min-height:0;display:grid;grid-template-columns:345px minmax(500px,1fr) 405px;gap:14px}.shot-panel{overflow:hidden}.shot-panel .panel-head{height:50px}.shot-list{padding:0 8px}.shot-list article{height:99px;display:grid;grid-template-columns:20px 98px 1fr 39px;gap:8px;align-items:center;border-bottom:1px solid #e5ebf4;padding:8px 3px;border-radius:8px}.shot-list article.selected{border:2px solid var(--blue);background:#f1f6ff}.grab{color:#6a7c9a;font-size:20px}.shot-thumb{height:80px;border-radius:6px;position:relative}.shot-thumb b{position:absolute;top:5px;left:5px;color:#fff;background:rgba(7,19,38,.78);padding:3px 5px;border-radius:4px}.shot-copy{min-width:0}.shot-copy>strong{font-size:16px}.shot-copy p{margin:5px 0;font-size:14px;color:#617697;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.shot-copy span{font-size:14px;display:flex;align-items:center;gap:6px}.shot-copy .dot{width:8px;height:8px}.shot-copy .dot.primary{background:var(--blue)}.shot-copy .dot.warning{background:var(--orange)}.shot-copy .dot.muted{background:#9aaac1}.shot-list time{align-self:end;padding-bottom:8px;color:#657794;font-size:14px}.preview-column{min-width:0;display:flex;flex-direction:column;gap:12px}.video-card{flex:1;min-height:0;overflow:hidden;display:flex;flex-direction:column}.video-preview{flex:1;position:relative;min-height:280px;background:#0a1728}.video-preview video{width:100%;height:100%;object-fit:contain;background:#060d18}.video-label,.timecode{position:absolute;z-index:2;top:9px;color:#fff;background:rgba(5,16,35,.78);border-radius:5px;padding:6px 9px;font-size:14px}.video-label{left:9px}.timecode{right:9px}.caption{position:absolute;bottom:14px;left:50%;transform:translateX(-50%);white-space:nowrap;background:rgba(3,14,29,.79);color:#fff;border-radius:5px;padding:7px 16px;font-size:19px;font-weight:650}.player{height:54px;display:flex;align-items:center;gap:13px;padding:0 16px}.player button{border:0;background:transparent;color:#142d57}.player span{color:#7082a0}.scrub,.volume{height:7px;border-radius:9px;background:#dce5f0;position:relative}.scrub{flex:1}.volume{width:62px}.scrub i,.volume i{position:absolute;height:100%;left:0;border-radius:inherit;background:var(--blue)}.version-row{height:58px;display:flex;gap:8px}.version-strip{display:flex;gap:6px;max-width:276px;overflow-x:auto}.version{width:86px;flex:0 0 86px;border:1px solid #d5e1ef;border-radius:9px;background:#fff;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding-left:13px}.version b{font-size:16px}.version span{font-size:14px;color:#677b9c}.version.active{border-color:var(--blue);background:#edf5ff;color:var(--blue)}.version.final-version{border-color:#8bd6b5;background:#f1fbf7;color:#11845d}.version-row>.btn{flex:1;padding:0 10px}.inspector{overflow:hidden}.inspector-tabs{height:44px;border-bottom:1px solid var(--line);display:grid;grid-template-columns:repeat(3,1fr)}.inspector-tabs button{border:0;background:transparent;font-weight:700;font-size:15px;position:relative}.inspector-tabs .active{color:var(--blue)}.inspector-tabs .active:after{content:"";position:absolute;left:22%;right:22%;bottom:0;height:3px;background:var(--blue)}.inspector-body{height:calc(100% - 44px);overflow-y:auto;padding:13px 16px}.mode-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin:9px 0 15px}.mode-grid button{height:65px;border:1px solid #d9e3f0;border-radius:7px;background:#fff;font-size:14px;color:#334b71}.mode-grid button span{display:block;font-size:19px;margin-bottom:5px}.mode-grid button.active{border-color:var(--blue);background:#edf5ff;color:var(--blue)}.field-head{display:flex;align-items:center;justify-content:space-between;margin:10px 0 7px}.field-head a{font-size:14px;color:var(--blue)}textarea{width:100%;height:73px;border:1px solid #d4dfef;border-radius:7px;resize:none;padding:11px;font-size:14px;line-height:1.6;background:#fff}.voice-row{display:flex;gap:7px}.segmented{gap:0;background:#eef3fa;border-radius:7px;padding:3px}.segmented button{height:35px;border:0;border-radius:6px;background:transparent;color:#53698d;font-size:14px}.segmented .active{background:#fff;color:var(--blue);font-weight:700}.select{flex:1;border:1px solid #d4dfef;border-radius:7px;background:#fff;display:flex;align-items:center;justify-content:center;gap:5px;font-size:14px}.round{width:38px;height:38px;border:1px solid #cfe0f5;border-radius:50%;display:grid;place-items:center;background:#fff;color:var(--blue)}.compact{min-height:38px;padding:0 8px;font-size:14px}.quality-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.quality-grid button{height:64px;border:1px solid #d8e2ef;border-radius:8px;background:#fff;text-align:left;padding:9px 12px}.quality-grid b,.quality-grid span{display:block}.quality-grid span{font-size:14px;color:#6c809f;margin-top:5px}.quality-grid .active{border-color:var(--blue);background:#eff6ff;color:var(--blue)}.audio-label{display:block;margin:13px 0 7px}.audio-setting{height:61px;border:1px solid #d8e2ef;border-radius:8px;display:flex;align-items:center;gap:10px;padding:8px 12px}.round.filled{width:27px;height:27px;background:var(--blue);color:#fff}.audio-setting>div{display:flex;flex-direction:column}.audio-setting span{font-size:14px;color:#7183a0;margin-top:3px}.audio-setting a{margin-left:auto;color:var(--blue);font-size:14px}.task-card{margin-top:16px;border:1px solid #dce5f0;border-radius:9px;padding:10px}.task-main{display:grid;grid-template-columns:72px 1fr 42px;gap:10px;align-items:center}.task-thumb{height:58px;border-radius:7px}.task-main b{font-size:14px}.task-main .progress{margin:7px 0}.task-main span{font-size:14px;color:#6c809d}.task-main>strong{color:#f25b20}.task-card footer{display:flex;gap:7px;align-items:center;border-top:1px solid #e8edf4;margin-top:9px;padding-top:8px;font-size:14px}.timeline{overflow:hidden}.timeline-toolbar{height:47px;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:10px;padding:0 18px}.timeline-toolbar>button{width:32px;height:32px;border:0;border-radius:50%;display:grid;place-items:center;background:var(--blue);color:#fff}.shutdown{margin-left:auto;margin-right:auto;background:#e6f8ef;color:#158b60;padding:6px 12px;border-radius:7px;font-size:14px}.shutdown .dot{display:inline-block;width:8px;height:8px;margin-right:7px}.zoom{margin-left:auto}.zoom button{height:32px;border:1px solid #d6e1ef;background:#fff;border-radius:7px}.ruler{height:27px;margin-left:113px;display:flex;justify-content:space-between;align-items:end;padding:0 12px 3px;font-size:14px;color:#607598;border-bottom:1px solid #dfe7f2}.tracks{height:202px;display:grid;grid-template-columns:113px 1fr}.track-labels{display:grid;grid-template-rows:repeat(4,1fr);border-right:1px solid var(--line)}.track-labels span{display:flex;align-items:center;gap:9px;padding-left:15px;border-bottom:1px solid #e5ebf3;font-weight:700;font-size:14px}.track-content{position:relative;display:grid;grid-template-rows:repeat(4,1fr);padding:6px 8px}.video-track,.voice-track,.subtitle-track{display:flex;gap:4px;min-width:0}.video-track i,.voice-track i,.subtitle-track i{font-style:normal;flex:1;min-width:0;border-radius:6px;padding:8px;color:#fff;font-size:14px;white-space:nowrap;overflow:hidden}.voice-track i{background:#ddf4ea;color:#27886b;text-align:center}.subtitle-track i{background:#ede6ff;color:#4d45c9;text-align:center}.music-track{margin-top:3px;border-radius:6px;background:#fff1d9;color:#dc7116;padding:9px;font-size:14px;white-space:nowrap;overflow:hidden}.playhead{position:absolute;left:42%;top:0;bottom:0;width:2px;background:#ff384f}.playhead:before{content:"";position:absolute;top:-1px;left:-4px;border-left:5px solid transparent;border-right:5px solid transparent;border-top:8px solid #ff384f}@media(max-width:1380px){.story-workspace{grid-template-columns:310px minmax(450px,1fr) 370px}.storyboard-page{grid-template-rows:84px minmax(0,1fr) 250px}.timeline{height:250px}.version-row .btn{font-size:14px}.format-pill{display:none}.shot-list article{grid-template-columns:17px 88px 1fr 34px}.shot-thumb{height:72px}.caption{font-size:16px}}
.shot-actions{display:flex;align-items:center;gap:3px}.shot-actions button{width:28px;height:28px;padding:0;border:0;border-radius:6px;display:grid;place-items:center;color:#506787;background:transparent}.shot-actions button:hover{color:var(--blue);background:#edf4ff}.shot-actions .add{width:34px;height:34px;margin-left:3px;border:1px solid #cfe0f3;background:#fff}.shot-list{height:calc(100% - 50px);overflow-y:auto}
.head-actions .btn:disabled,.version-row .btn:disabled,.version:disabled{opacity:.58;cursor:not-allowed}.spin{animation:spin .9s linear infinite}.task-cancel{border:0;background:transparent;color:#d6463c;display:flex;align-items:center;gap:4px;font-size:14px}.task-main .task-error{color:#c53d35}.task-main .progress i{display:block;height:100%;border-radius:inherit;background:var(--blue);transition:width .25s ease}@keyframes spin{to{transform:rotate(360deg)}}
.task-main .progress.indeterminate{overflow:hidden}.task-main .progress.indeterminate i{width:36%;animation:task-sweep 1.4s ease-in-out infinite}@keyframes task-sweep{from{transform:translateX(-110%)}to{transform:translateX(310%)}}
.empty-storyboard{height:calc(100% - 50px);padding:28px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;gap:10px;color:#617697}.empty-storyboard b{color:#29466f}.empty-storyboard span{max-width:245px;font-size:14px;line-height:1.6}.empty-storyboard .btn{margin-top:7px}
.reference-inputs{margin:-3px 0 12px;padding:9px 10px;border:1px solid #d9e4f1;border-radius:8px;background:#f8fbff;display:grid;grid-template-columns:1fr 1fr;gap:7px}.reference-inputs .field-head{grid-column:1/-1;margin:0 0 2px}.reference-inputs .field-head a{color:var(--blue);font-size:14px}.reference-inputs label{display:flex;flex-direction:column;gap:4px;color:#52698c;font-size:14px}.reference-inputs select{height:34px;min-width:0;border:1px solid #cfdced;border-radius:6px;background:#fff;padding:0 8px;color:#203b65}.reference-inputs p{grid-column:1/-1;color:#7385a2;font-size:14px}
.narration-import-row{display:grid;grid-template-columns:1fr auto auto;gap:7px;align-items:center;margin-top:7px}.narration-import-row .select{height:34px}.narration-import-row a{color:var(--blue);font-size:14px;white-space:nowrap}
.narration-modes{display:grid;grid-template-columns:repeat(3,1fr);margin-bottom:8px}.narration-off-note{padding:14px;border:1px solid var(--line);border-radius:8px;background:var(--surface-soft);color:var(--muted);font-size:14px;line-height:1.6}.ambient-field{margin-bottom:8px}.ambient-field textarea{height:58px}.audio-policy-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:6px}.audio-policy-grid button{min-height:68px;padding:8px;border:1px solid var(--line);border-radius:8px;background:var(--surface);text-align:left;color:var(--text)}.audio-policy-grid b,.audio-policy-grid span{display:block}.audio-policy-grid span{margin-top:4px;color:var(--muted);font-size:14px;line-height:1.35}.audio-policy-grid button.active{border-color:var(--blue);background:#edf5ff;color:var(--blue)}
.locked-note{margin-bottom:10px;padding:9px 11px;border:1px solid #efc36a;border-radius:7px;display:flex;align-items:center;gap:7px;background:#fff8e8;color:#8b5a08;font-size:14px}.scene-locked>:not(.locked-note){opacity:.58;pointer-events:none}.shot-actions button:disabled{opacity:.35;cursor:not-allowed}.prompt-mode{display:flex;gap:3px;padding:2px;border-radius:7px;background:#eef3f9}.prompt-mode button{height:28px;border:0;border-radius:5px;padding:0 9px;background:transparent;color:var(--muted);font-size:14px}.prompt-mode button.active{background:#fff;color:var(--blue);box-shadow:0 1px 4px #cbd8e9}.intent-editor{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px}.intent-editor label{display:flex;flex-direction:column;gap:5px;color:var(--text);font-size:14px;font-weight:700}.intent-editor input,.intent-editor textarea{min-width:0;border:1px solid var(--line);border-radius:7px;background:var(--surface);padding:8px;color:var(--text)}.intent-editor input{height:36px}.intent-editor textarea{height:64px;resize:vertical}.intent-editor .wide{grid-column:1/-1}.prompt-preview{padding:9px 10px;border:1px solid #cfe0f7;border-radius:8px;background:#f6faff;font-size:14px}.prompt-preview p{margin:5px 0;color:#405a80;line-height:1.5}.prompt-preview span{color:#b36a15}.prompt-preview .success-text{color:#13835a}.audio-intents{display:grid;grid-template-columns:repeat(3,1fr);margin-bottom:9px}
.full-preview-backdrop{position:fixed;z-index:95;inset:60px 0 0;display:grid;place-items:center;background:rgba(5,18,40,.72);backdrop-filter:blur(3px)}.full-preview-dialog{width:min(1050px,82vw);overflow:hidden;border:1px solid #6d7f9b;border-radius:13px;background:#071326;box-shadow:0 26px 80px rgba(0,0,0,.38)}.full-preview-dialog header{height:68px;padding:0 18px;display:flex;align-items:center;justify-content:space-between;color:#fff;background:#0d1c33}.full-preview-dialog header h2{color:#fff}.full-preview-dialog header p{margin-top:4px;color:#aab9cf;font-size:14px}.full-preview-dialog header button{width:38px;height:38px;border:0;border-radius:8px;display:grid;place-items:center;color:#d7e2f1;background:transparent}.full-preview-dialog header button:hover{background:#1a2d49}.full-preview-dialog video{display:block;width:100%;max-height:calc(82vh - 98px);aspect-ratio:16/9;object-fit:contain;background:#000}
.empty-player-state{height:54px;padding:0 16px;display:flex;align-items:center;color:#7184a3;font-size:14px;background:linear-gradient(90deg,#fbfdff,#f6f9fd)}
@media(max-height:850px){.storyboard-page{grid-template-rows:74px minmax(0,1fr) 200px}.timeline{height:200px}.tracks{height:126px}.video-preview{min-height:170px}.empty-player-state{height:42px}.version-row{height:50px}.story-head .compute-status{display:none}}
.shot-thumb{overflow:hidden;background:#e9f0f8}.shot-thumb video{width:100%;height:100%;display:block;object-fit:cover}.shot-thumb>span{width:100%;height:100%;display:grid;place-items:center;color:#7c8fae}.empty-preview{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;color:#8395af;background:#0a1728}.empty-preview b{font-size:18px;color:#e2eaf5}.empty-preview span{font-size:14px}.task-thumb{display:grid;place-items:center;background:#eaf2fb;color:#5c7da8}.video-track i{position:relative;overflow:hidden;background:#203b64}.video-track i video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover}.video-track i>span{position:relative;z-index:1;text-shadow:0 1px 4px #07162c}.video-track i:not(:has(video)){background:#dbe6f3;color:#496482}.video-track i:not(:has(video))>span{text-shadow:none}
.inspector-empty{height:calc(100% - 44px);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;color:var(--muted)}.editor-field{display:flex;flex-direction:column;gap:7px;margin-bottom:14px;font-size:14px;font-weight:700}.editor-field input,.editor-field textarea{width:100%;border:1px solid #d4dfef;border-radius:7px;background:var(--surface);padding:9px 11px}.editor-field input{height:42px}.editor-field textarea{height:80px}.editor-field.visual-plan textarea{height:148px;font-size:14px}.editor-field input:focus,.editor-field textarea:focus{outline:2px solid #d7e8ff;border-color:var(--blue)}.visual-summary{margin-top:14px;padding:12px;border:1px solid var(--line);border-radius:8px;background:var(--surface-soft);display:flex;flex-direction:column;gap:6px}.visual-summary span{font-size:14px;color:var(--muted)}.duration-options{display:grid;grid-template-columns:repeat(3,1fr);margin-bottom:15px}.timeline-note{margin-left:auto;color:var(--muted);font-size:14px}.timeline-toolbar>button:disabled{opacity:.5;cursor:not-allowed}
.prompt-preview .ready{color:#13835a}
.candidate-detail{min-height:52px;padding:7px 11px;border:1px solid var(--line);border-radius:8px;background:var(--surface);display:flex;flex-direction:column;justify-content:center;gap:5px}.candidate-detail>div{display:flex;align-items:center;gap:10px;font-size:14px;color:var(--muted)}.candidate-detail>div b{max-width:210px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--text)}.candidate-detail>div span{padding-left:10px;border-left:1px solid var(--line)}.candidate-detail p{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#476184;font-size:14px}
.prompt-controls{display:flex;align-items:center;gap:7px}.ai-revise{height:30px;padding:0 9px;border:1px solid #bfd6f6;border-radius:6px;background:#f3f8ff;color:var(--blue);display:flex;align-items:center;gap:5px;font-size:14px;font-weight:700}.ai-revise:disabled{opacity:.5;cursor:not-allowed}.revision-backdrop{position:fixed;z-index:110;inset:60px 0 0;background:rgba(5,20,45,.48);display:grid;place-items:center;padding:24px}.revision-dialog{width:min(860px,calc(100vw - 80px));max-height:calc(100vh - 90px);overflow:auto;border:1px solid #cbd9eb;border-radius:13px;background:var(--surface);box-shadow:0 26px 80px rgba(8,32,70,.28)}.revision-dialog>header{min-height:82px;padding:17px 21px;border-bottom:1px solid var(--line);display:flex;align-items:flex-start;justify-content:space-between}.revision-dialog>header p{margin-top:6px;color:var(--muted);font-size:14px}.revision-dialog>header button{width:34px;height:34px;border:0;border-radius:7px;background:transparent;color:var(--text);display:grid;place-items:center}.revision-body{padding:18px 21px;display:flex;flex-direction:column;gap:14px}.revision-body>label{display:flex;flex-direction:column;gap:7px;font-weight:700}.revision-body>label textarea{height:100px}.revision-compare{display:grid;grid-template-columns:1fr 1fr;gap:12px}.revision-compare article{min-height:188px;padding:14px;border:1px solid var(--line);border-radius:9px;background:var(--surface-soft)}.revision-compare article.proposed{border-color:#9ec3f6;background:#f1f7ff}.revision-compare h3{margin:8px 0;font-size:16px}.revision-compare p{min-height:72px;color:#405a80;font-size:14px;line-height:1.55;white-space:pre-wrap}.revision-compare small{display:block;margin-top:9px;color:var(--muted);line-height:1.5}.revision-summary{padding:10px 12px;border-radius:8px;background:#eaf7f1;color:#196e51;display:flex;flex-direction:column;gap:5px}.revision-summary span{font-size:14px}.revision-error{padding:9px 11px;border-radius:7px;background:#fff0ee;color:#b23c36;font-size:14px}.revision-dialog>footer{min-height:68px;padding:11px 21px;border-top:1px solid var(--line);display:flex;justify-content:flex-end;gap:9px}
.duration-picker{position:relative;margin:0 0 15px;padding-right:78px}.duration-picker>input[type="range"]{width:100%;height:26px;accent-color:var(--blue)}.duration-scale{display:flex;justify-content:space-between;color:var(--muted);font-size:14px}.duration-picker>label{position:absolute;right:0;top:0;width:66px;height:42px;border:1px solid var(--line);border-radius:8px;background:var(--surface-soft);display:flex;align-items:center;justify-content:center;gap:3px}.duration-picker>label input{width:34px;border:0;background:transparent;color:var(--text);font-size:16px;font-weight:800;text-align:right;outline:none}.duration-picker>label span{color:var(--muted);font-size:14px}
...
.pool-pill{height:49px;padding:0 10px;border:1px solid var(--line);border-radius:9px;background:var(--surface);display:flex;align-items:center;gap:7px;color:var(--muted);font-size:14px;white-space:nowrap}.pool-pill select{height:30px;border:1px solid #cbd9eb;border-radius:6px;background:var(--surface-soft);padding:0 7px;color:var(--text);font-size:14px;font-weight:700}.pool-pill.active{border-color:#9fc3f4;background:#f2f7ff;color:var(--blue)}
@media(max-width:1380px){.pool-pill>span{display:none}.pool-pill{padding:0 7px}}
.generation-speed{height:49px;position:relative;z-index:12}.generation-speed summary{height:49px;min-width:138px;padding:0 12px;border:1px solid var(--line);border-radius:9px;background:var(--surface);display:flex;flex-direction:column;align-items:flex-start;justify-content:center;gap:3px;cursor:pointer;list-style:none}.generation-speed summary::-webkit-details-marker{display:none}.generation-speed summary span{color:var(--muted);font-size:14px}.generation-speed summary b{font-size:14px}.generation-speed.active summary{border-color:#9fc3f4;background:var(--blue-soft);color:var(--blue)}.speed-menu{position:absolute;top:56px;right:0;width:380px;padding:13px;border:1px solid var(--line);border-radius:11px;background:var(--surface);box-shadow:0 18px 48px rgba(15,38,73,.22)}.speed-menu header{padding:2px 3px 10px;display:flex;flex-direction:column;gap:4px}.speed-menu header b{font-size:16px}.speed-menu header span{color:var(--muted);font-size:14px}.speed-menu>button{width:100%;min-height:72px;margin-top:7px;padding:9px 10px;border:1px solid var(--line);border-radius:8px;background:var(--surface-soft);display:grid;grid-template-columns:20px 1fr;gap:9px;text-align:left;color:var(--text)}.speed-menu>button>i{width:17px;height:17px;margin-top:3px;border:2px solid #9aabc1;border-radius:50%;position:relative}.speed-menu>button.selected{border-color:var(--blue);background:var(--blue-soft)}.speed-menu>button.selected>i{border-color:var(--blue)}.speed-menu>button.selected>i:after{content:"";position:absolute;inset:3px;border-radius:50%;background:var(--blue)}.speed-menu>button>span{min-width:0;display:grid;grid-template-columns:1fr auto;gap:4px 10px}.speed-menu>button b{font-size:15px}.speed-menu>button small{color:var(--blue);font-size:14px;font-weight:700}.speed-menu>button em{grid-column:1/3;color:var(--muted);font-size:14px;font-style:normal;line-height:1.4}.speed-plan{margin-top:10px;padding:9px 10px;border:1px solid var(--line);border-radius:7px;background:var(--surface-soft);display:flex;flex-direction:column;gap:3px}.speed-plan b{font-size:14px}.speed-plan span{color:var(--muted);font-size:14px;line-height:1.4}.speed-menu footer{margin-top:8px;padding:9px 10px;border-radius:7px;background:var(--warning-soft);color:#a45a13;font-size:14px;line-height:1.45}:global(:root[data-theme="dark"]) .speed-menu footer{color:#ffc078}:global(:root[data-theme="dark"]) .generation-speed.active summary{border-color:var(--blue);background:var(--selected);color:#9fc3ff}
@media(max-width:1380px){.generation-speed summary{min-width:118px}.generation-speed summary span{display:none}.speed-menu{width:350px}.field-head{align-items:stretch;flex-direction:column;gap:8px}.prompt-controls{justify-content:flex-end}.ai-revise,.prompt-mode button{white-space:nowrap}}
</style>
