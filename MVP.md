...
- 参考素材标签。
- 禁止元素。
- `integrated_multimodal_description`：按镜头时间线描述画面、动作、运镜和参考素材，不包含系统旁白或字幕文字，并明确不生成人声。
...
- [ ] H3 候选默认保留原生环境音；“智能使用”会检查音轨并在疑似人声、削波或异常静音时停用整轨，用户仍可选择始终使用或不使用。
...
- 资料页的目标受众与目标时长来自当前项目，并作为 DeepSeek 提取知识点和生成分镜的真实参数；资料原文不再任意高亮固定段落。
...
- 设置页仅展示已经接入的算力与连接、DeepSeek、存储和主题能力。空闲 3 分钟和 GPU 运行 60 分钟上限作为当前固定有效策略展示；尚无可靠平台接口的七天保活继续保持未启用，避免出现可点击但无效果的开关。
- 当前下一步是用真实新建项目在桌面 EXE 内完成“导入资料 → DeepSeek 分镜 → 视觉规范 → 素材/旁白 → 提交候选 → 选择正式版本 → 导出”的逐页验收。到提交候选之前均不需要启动 5090。

### 22.7 2026-09-12 干净实例与真实内容链路验收

- 使用用户授权的 DeepSeek 凭据真实完成“资料原文 → 3～12 条带来源知识点 → 人工确认 → 5/10/15 秒分镜规划 → SQLite 分镜持久化”。两次模型调用约 95 秒；分镜输出的字段校验错误现会指出具体镜头和原因，便于用户修正或重新生成。
...
- 桌面 `ServiceClient` 已真实完成 Qwen Image 关键帧生成、鉴权下载、将生成图重新上传为首帧、H3 I2V Turbo 候选提交、轮询和校验下载。完整复测约 196.6 秒；关键帧为 1344×768 PNG，候选为 1344×756、24fps、5.17 秒 H.264，且无音轨，符合默认丢弃 H3 原生音频的产品约束。
